" Vim syntax file
" Language: Terraform
" Acknowledgment: https://github.com/hashivim/vim-terraform/blob/master/syntax/terraform.vim
...
syntax keyword terraDataTypeBI
...
""" resource
syntax keyword terraResourceTypeBI
          \ aci_access_port_block
          \ aci_access_port_selector
          \ aci_application_epg
          \ aci_application_profile
...
syntax keyword terraSection connection output variable terraform locals
syntax keyword terraValueBool true false on off yes no

syntax keyword terraTodo         contained TODO FIXME XXX BUG
syntax region  terraComment      start="/\*" end="\*/" contains=terraTodo,@Spell
syntax region  terraComment      start="#" end="$" contains=terraTodo,@Spell
syntax region  terraComment      start="//" end="$" contains=terraTodo,@Spell

syntax keyword terraResource        resource nextgroup=terraResourceTypeStr skipwhite
syntax region  terraResourceTypeStr start=/"/ end=/"/ contains=terraResourceTypeBI nextgroup=terraResourceName skipwhite
syntax region  terraResourceName    start=/"/ end=/"/ nextgroup=terraResourceBlock skipwhite

syntax keyword terraData        data nextgroup=terraDataTypeStr skipwhite
syntax region  terraDataTypeStr start=/"/ end=/"/ contains=terraDataTypeBI nextgroup=terraDataName skipwhite
syntax region  terraDataName    start=/"/ end=/"/ nextgroup=terraDataBlock skipwhite

""" provider
syntax keyword terraProvider     provider nextgroup=terraProviderName skipwhite
syntax region  terraProviderName start=/"/ end=/"/ nextgroup=terraProviderBlock skipwhite

""" provisioner
syntax keyword terraProvisioner     provisioner nextgroup=terraProvisionerName skipwhite
syntax region  terraProvisionerName start=/"/ end=/"/ nextgroup=terraProvisionerBlock skipwhite

""" module
syntax keyword terraModule     module nextgroup=terraModuleName skipwhite
syntax region  terraModuleName start=/"/ end=/"/ nextgroup=terraModuleBlock skipwhite

""" dynamic (HCL2)
syntax keyword terraDynamic     dynamic nextgroup=terraDynamicName skipwhite
syntax region  terraDynamicName start=/"/ end=/"/ nextgroup=terraDynamicBlock skipwhite

""" misc.
syntax match terraValueDec      "\<[0-9]\+\([kKmMgG]b\?\)\?\>"
syntax match terraValueHexaDec  "\<0x[0-9a-f]\+\([kKmMgG]b\?\)\?\>"
syntax match terraBraces        "[\[\]]"

""" skip \" and \\ in strings.
syntax region terraValueString   start=/"/ skip=/\\\\\|\\"/ end=/"/ contains=terraStringInterp
syntax region terraStringInterp  matchgroup=terraBraces start=/\${/ end=/}/ contained contains=ALL
syntax region terraHereDocText   start=/<<-\?\z([a-z0-9A-Z]\+\)/ end=/^\s*\z1/ contains=terraStringInterp

"" Functions.
syntax match terraValueFunction "[a-z0-9]\+(\@="

""" HCL2
syntax keyword terraContent        content
syntax keyword terraRepeat         for in
syntax keyword terraConditional    if
syntax keyword terraPrimitiveType  string bool number
syntax keyword terraStructuralType object tuple
syntax keyword terraCollectionType list map set
syntax keyword terraValueNull      null

""" Terraform v0.12
syntax keyword terraTodo contained TF-UPGRADE-TODO

" enable block folding
syntax region terraBlock matchgroup=terraBraces start="{" end="}" fold transparent

highlight def link terraComment           Comment
highlight def link terraTodo              Todo
highlight def link terraProvider          Structure
highlight def link terraBraces            Delimiter
highlight def link terraProviderName      String
highlight def link terraResource          Structure
highlight def link terraResourceName      String
highlight def link terraResourceTypeBI    Tag
highlight def link terraResourceTypeStr   String
highlight def link terraData              Structure
highlight def link terraDataName          String
highlight def link terraDataTypeBI        Tag
highlight def link terraDataTypeStr       String
highlight def link terraSection           Structure
highlight def link terraValueBool         Boolean
highlight def link terraValueDec          Number
highlight def link terraValueHexaDec      Number
highlight def link terraValueString       String
highlight def link terraHereDocText       String
highlight def link terraProvisioner       Structure
highlight def link terraProvisionerName   String
highlight def link terraModule            Structure
highlight def link terraModuleName        String
highlight def link terraValueFunction     Function
highlight def link terraDynamic           Structure
highlight def link terraDynamicName       String
highlight def link terraContent           Structure
highlight def link terraRepeat            Repeat
highlight def link terraConditional       Conditional
highlight def link terraPrimitiveType     Type
highlight def link terraStructuralType    Type
highlight def link terraCollectionType    Type
highlight def link terraValueNull         Constant
